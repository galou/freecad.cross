...
def coin_from_planning_scene_msg(
    ...
    separator.addChild(
        coin_from_planning_scene_world(
            scene.world,
...
def str_from_pose(pose: Pose) -> str:
    ...
    """
    return (
        f'({pose.position.x}, {pose.position.y}, {pose.position.z} m;'
...
def box_from_primitive(primitive: SolidPrimitive) -> coin.SoSeparator:
    ...
    if primitive.type != SolidPrimitive.BOX:
        raise ValueError(
            'Unsupported SolidPrimitive type, got {primitive.type},'
