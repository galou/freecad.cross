...
def _prefix_names_with_macro_param(xml: et.Element) -> None:
    """Prefix all URDF name references with `${prefix}` in-place."""
    for attr in ('name', 'link', 'joint'):
        if attr not in xml.attrib:
            continue
        value = xml.attrib[attr]
        if value.startswith('${prefix}'):
            continue
        xml.attrib[attr] = f'${{prefix}}{value}'
    for child in xml:
        _prefix_names_with_macro_param(child)
...
class RobotProxy(ProxyBase):
    ...
    def export_urdf(self, interactive: bool = False) -> Optional[et.Element]:
        ...
        xml = self._build_urdf_xml(package_parent, package_name)
        if xml is None:
            return None
    ...
    def export_xacro(self, interactive: bool = False) -> Optional[et.Element]:
        """Export the robot as xacro with macro and wrapper files."""
        if not self.is_execute_ready():
            return None
        if not self.robot.OutputPath:
            warn('Property `OutputPath` cannot be empty', True)
        ...
        for child in urdf_xml:
            ...
            _prefix_names_with_macro_param(prefixed_child)
            macro_et.append(prefixed_child)

        xacro_xml = et.fromstring('<robot/>')
